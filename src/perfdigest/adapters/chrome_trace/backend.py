...
CHROME_TRACE_USAGE = (
    ...
    "ranking, and likewise [dispatch] units: torch.compile's own envelope spans "
    "(Torch-Compiled Region, CompiledFunction/CompiledFunctionBackward, "
    "TorchDynamo Cache Lookup, AOTDispatcher wrappers, Inductor's "
    "'## Call CompiledFxGraph ##'), each of which CONTAINS the compiled work "
    "rather than being it. Overlapping the same way but deliberately untagged: "
    "eager autograd envelopes named 'autograd::engine::evaluate_function: X' "
    "always contain their node X. "
    "When comparing two traces whose capture lengths differ, compare "
    "avg_time_us (calls-normalized), not total_time_us. COMPARING EAGER VS "
    "COMPILED: fusion changes how many kernels exist, so check the unit counts "
    "on each side before trusting a single kernel_b pairing — if the compiler "
    "merged or split kernels, sum each side's group instead. Measured example: "
    "one individually-correct pairing read +101% (a regression) where the true "
    "3-kernel vs 2-kernel groups were -12.3% (an improvement) — the SIGN was "
    "wrong, not merely the magnitude. For hardware counters on "
